...
def get_polar_data(file):
    ...
    cosines = (
        np.array(range(int(K), -int(K + 1), -1)) * del_cos
    )  # вектор косинусов от 1 до -1
    ...
    delta_phi = math.pi / 2
    phi_q = np.linspace(
        math.pi / 2 - delta_phi, math.pi / 2 + delta_phi, 200
    )  # вектор углов для интерполяции
