...
class Project(models.Model):
    ...
    max_intensity = models.FloatField('максимальная допустимая интенсивность', default=10)

    def __str__(self):
        return self.project_name

    def track_intensity(self):
        date_intensity = {measure.date: measure.mean_intensity for measure in self.measure_set.all()}
        return pd.DataFrame({'date': date_intensity.keys(), 'intensity': date_intensity.values()})
...
class Measure(models.Model):
    ...
    measure_name = models.CharField('название измерения', max_length=150, default='дефектовка')
