...
def addmeasure(request, project_id):
    if request.method == "POST":
        ...
        mean_intensity = processed_data.query("@start <= phi <= @stop").r.mean()
        now = timezone.now()
        ...
        form = MeasureForm()
    return render(request, "factory/addmeasure.html", {"form": form})
...
def model_form_upload(request, project_id):
    if request.method == "POST":
        ...
        form = MeasureForm()
    return render(request, "factory/add_measure_.html", {"form": form})
